...
def purchase(List):
    ...
    while flag.upper()=="Y":  #check and go if flag is 'Y' or 'y'.
        ...
        if product_name==L[0][0].upper() or product_name==L[1][0].upper() or product_name==L[2][0].upper(): ##or product_name==L[3][0].upper(): #check the user entered product name with stock of store 
            p=True
            while p==True:
                try:
...
##                    q[product_name]=p_quantity                                                       
            ...
            for i in range(len(L)):
                print(L[i][0],"\t\t",L[i][1],"\t\t",L[i][2])##,"\t\t",L[i][3]) # print, last updated product name, quantity and price.
            print("--------------------------------------------")
    ...
    for keys in q.keys():
        ...
        else: ##keys==L[2][0].upper():   #executes this operation if product is hdd entered by customer.
            h_price=int(L[2][1])
            h_num=int(q[keys])
            h_amount=(h_price*h_num)
            f_amount+=(h_price*h_num)
            print("Total cost for HDD: ",h_amount)
##        else:                        #executes this operation if product is g entered by customer.
##              d_price=int(L[3][1])
##              d_num=int(q[keys])
##              d_amount=(d_price*d_num)
##              f_amount+=(d_price*d_num)
######                print("Total cost for g: ",d_amount)  
...
##            else:
##              file.write(str("\n"+str(keys)+" \t\t "+str(q['g'])+" \t\t "+str(L[2][1])+" \t\t "+str(d_amount)))  
